"""Controller for the project pipeline screen: start/stop via the application layer."""
...
class ProjectPipelineController(BaseScreenController):
    """Coordinates the pipeline screen: start and cancel through the application layer."""
    ...
    def __init__(
        ...
        *,
        ...
        # Incremented on each Run; drops UI callbacks from a previous run so
        # on_run_done / stage updates after Stop → Run do not corrupt panels.
    ...
    def set_project(self, project_root: Path) -> None:
        """Show the execution screen for a project without auto-starting the pipeline."""
    ...
    def _on_run_done(
        ...
        """Invoked when the pipeline finishes; updates the UI on the GUI thread."""
    ...
    def _handle_run(self) -> None:
        """Start (or restart) the pipeline from this screen."""
        ...
        if self._pipeline_storage.get_current() is not None:
            if self._layout and self._layout.modals:
                self._layout.modals.show_info(
                    locmsg("project.run_modal.title"),
                    locmsg("project.run_modal.busy_message"),
                )
            return
    ...
    def _handle_stop(self) -> None:
        ...
        self._publish_pipeline_line(
            "[INFO] Stop requested; waiting for current steps to finish..."
        )
    ...
    def _publish_pipeline_line(
        ...
        """Publish a line to the console. ``ui_run_epoch`` avoids panel updates from an old run."""
    ...
    def _on_pipeline_progress(
        ...
        """Pipeline progress events (not log text); GUI updates via ``call_in_ui``."""
        if self._project_console is None:
            return
        if stage_id == "extract":
        ...
        if stage_id == "result":
            ...
            if not isinstance(done, int) or not isinstance(total, int):
                return
            self._project_console.call_in_ui(
                lambda e=run_epoch, d=done, t=total: self._apply_result_progress(
                    e, d, t
                )
            )
            return
    ...
    def _apply_result_progress(
        ...
        if run_epoch != self._pipeline_run_epoch:
            return
        self._view.set_result_documents_progress(done, total)
    ...
    def _on_stage_complete(
        ...
        """Called from the pipeline thread after a successful stage; GUI only via ``call_in_ui``."""
        if self._project_console is None:
            return
