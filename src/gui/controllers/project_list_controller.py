"""Контроллер экрана списка проектов."""
...
class ProjectListController(BaseScreenController):
    """Координирует экран списка проектов: загрузка списка, создание/удаление через adapters."""
    ...
    def screen_title(self) -> str:
        return ProjectListScreen.SCREEN_TITLE
    ...
    def on_show(self) -> None:
        try:
            ...
            SYSTEM_LOGGER.exception("Ошибка при обновлении списка проектов: %s", exc)
        ...
        try:
            ...
            if self._layout and self._layout.modals:
                self._layout.modals.show_info(
                    "Ошибка", "", errors=[f"Проект с именем '{name}' уже существует."]
                )
        except ValueError as e:
            if self._layout and self._layout.modals:
                self._layout.modals.show_info("Ошибка", "", errors=[str(e)])
    ...
    def _on_delete_project(self, info) -> None:
        try:
            ...
            self.on_show()
            if self._layout and self._layout.modals:
                self._layout.modals.show_info("Удаление", "Проект уже удалён.")
        except ValueError as exc:
            if self._layout and self._layout.modals:
                self._layout.modals.show_info("Ошибка", "", errors=[str(exc)])
        except Exception as exc:
            if self._layout and self._layout.modals:
                self._layout.modals.show_info(
                    "Удаление", "", errors=[f"Не удалось удалить проект: {exc}"]
                )
