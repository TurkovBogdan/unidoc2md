"""Controller for project settings: save, validation, run via the application layer."""
...
class ProjectConfigController(BaseScreenController):
    """Coordinates the project config screen: load/save through application, navigation to run."""
    ...
    def load_config(self) -> None:
        """Reload config into the form (e.g. when reopening the screen)."""
    ...
    def _handle_save(self) -> None:
        ...
        if not ok and self._layout and self._layout.modals:
            self._layout.modals.show_info(
                locmsg("project.save_modal.title"),
                locmsg("project.save_modal.validation_message"),
                errors=errors,
            )
        elif ok and self._layout and self._layout.modals:
            self._layout.modals.show_info(
                locmsg("project.save_modal.title"),
                locmsg("project.save_modal.success_message"),
            )
