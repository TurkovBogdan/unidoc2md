"""Pipeline tab: stage thread counts and OCR hint banner."""
...
class PipelineSettingsTab(ttk.Frame):
    """Pipeline-wide thread spinboxes and optional Yandex OCR banner."""

    SETTINGS_WIDTH_PX = 520
    _SEPARATOR_PADX = 12

    def _bool_to_display(self, b: bool) -> str:
        return self._bool_ui[0] if b else self._bool_ui[1]

    def _display_to_bool(self, display: str, default: bool) -> bool:
        d = (display or "").strip()
    ...
    def _build_ui(self) -> None:
        ...
        self._article_text.insert(tk.END, locmsg("project_pipeline.article"))
        ...
        block.add_comment(self._desc_create_index)
        ...
        block.add_comment(self._desc_threads_section)
        ...
        self._lbl_extract = gui_element_input_label(
            ex_row,
            locmsg("project_pipeline.extract_threads_label"),
            wraplength=cfg["column_label_px"],
        )
        block.finish_field_row(ex_row, self._lbl_extract, self._extract_threads_spin)
        self._desc_extract = gui_element_input_description(
            block.form,
            locmsg("project_pipeline.extract_threads_hint"),
            wraplength=cfg["container_width_px"],
        )
        block.add_comment(self._desc_extract)
        ...
        self._lbl_image = gui_element_input_label(
            img_row,
            locmsg("project_pipeline.image_processing_threads_label"),
            wraplength=cfg["column_label_px"],
        )
        block.finish_field_row(img_row, self._lbl_image, self._image_processing_threads_spin)
        self._desc_image = gui_element_input_description(
            block.form,
            locmsg("project_pipeline.image_processing_threads_hint"),
            wraplength=cfg["container_width_px"],
        )
        block.add_comment(self._desc_image)

        self._pipeline_image_processing_banner = gui_element_warning_banner(
            block.form,
            locmsg("project_pipeline.yandex_ocr_banner"),
        )
        ...
        self._lbl_markdown = gui_element_input_label(
            md_row,
            locmsg("project_pipeline.markdown_threads_label"),
            wraplength=cfg["column_label_px"],
        )
        block.finish_field_row(md_row, self._lbl_markdown, self._markdown_threads_spin)
        self._desc_markdown = gui_element_input_description(
            block.form,
            locmsg("project_pipeline.markdown_threads_hint"),
            wraplength=cfg["container_width_px"],
        )
        block.add_comment(self._desc_markdown)
        ...
        self._lbl_tagging = gui_element_input_label(
            tag_row,
            locmsg("project_pipeline.tagging_threads_label"),
            wraplength=cfg["column_label_px"],
        )
        block.finish_field_row(tag_row, self._lbl_tagging, self._tagging_threads_spin)
        self._desc_tagging = gui_element_input_description(
            block.form,
            locmsg("project_pipeline.tagging_threads_hint"),
            wraplength=cfg["container_width_px"],
        )
        block.add_comment(self._desc_tagging)

    def refresh_locale(self) -> None:
        """Refresh form strings after language change."""
        try:
            old_yes, old_no = self._bool_ui
            cur = (self._create_documents_index_var.get() or "").strip()
            if cur == old_yes:
                idx_bool = True
            elif cur == old_no:
                idx_bool = False
            else:
                idx_bool = CREATE_DOCUMENTS_INDEX_DEFAULT
            self._bool_ui = (locmsg("gui.yes"), locmsg("gui.no"))
            self._create_documents_index_var.set(self._bool_to_display(idx_bool))
            if self._idx_combo is not None:
                self._idx_combo.set_values(list(self._bool_ui))

            if self._article_title_label is not None and self._article_title_label.winfo_exists():
                self._article_title_label.configure(text=locmsg("project_pipeline.notes_title"))
            self._article_text.configure(state=tk.NORMAL)
            ...
            if self._desc_create_index is not None:
                self._desc_create_index.configure(
                    text=locmsg("project_pipeline.create_documents_index_hint"),
                    wraplength=cfg["container_width_px"],
                )
            if self._hdr_threads is not None:
                self._hdr_threads.configure(text=locmsg("project_pipeline.threads_section_title"))
            if self._desc_threads_section is not None:
                self._desc_threads_section.configure(
                    text=locmsg("project_pipeline.threads_section_description"),
                    wraplength=cfg["container_width_px"],
                )
            if self._lbl_extract is not None:
                self._lbl_extract.configure(
                    text=locmsg("project_pipeline.extract_threads_label"),
                    wraplength=cfg["column_label_px"],
                )
            if self._desc_extract is not None:
                self._desc_extract.configure(
                    text=locmsg("project_pipeline.extract_threads_hint"),
                    wraplength=cfg["container_width_px"],
                )
            if self._lbl_image is not None:
                self._lbl_image.configure(
                    text=locmsg("project_pipeline.image_processing_threads_label"),
                    wraplength=cfg["column_label_px"],
                )
            if self._desc_image is not None:
                self._desc_image.configure(
                    text=locmsg("project_pipeline.image_processing_threads_hint"),
                    wraplength=cfg["container_width_px"],
                )
            if self._lbl_markdown is not None:
                self._lbl_markdown.configure(
                    text=locmsg("project_pipeline.markdown_threads_label"),
                    wraplength=cfg["column_label_px"],
                )
            if self._desc_markdown is not None:
                self._desc_markdown.configure(
                    text=locmsg("project_pipeline.markdown_threads_hint"),
                    wraplength=cfg["container_width_px"],
                )
            if self._lbl_tagging is not None:
                self._lbl_tagging.configure(
                    text=locmsg("project_pipeline.tagging_threads_label"),
                    wraplength=cfg["column_label_px"],
                )
            if self._desc_tagging is not None:
                self._desc_tagging.configure(
                    text=locmsg("project_pipeline.tagging_threads_hint"),
                    wraplength=cfg["container_width_px"],
                )
            banner = self._pipeline_image_processing_banner
            if banner is not None and banner.winfo_exists():
                children = banner.winfo_children()
                if len(children) >= 2:
                    children[1].configure(text=locmsg("project_pipeline.yandex_ocr_banner"))
        except tk.TclError:
            pass
    ...
    def load_pipeline(self, data: dict[str, Any] | None) -> None:
        """Load widgets from pipeline section data."""
    ...
    def get_pipeline_data(self) -> dict[str, Any]:
        """Collect pipeline section from widgets."""
        default = PipelineConfig.get_default()
