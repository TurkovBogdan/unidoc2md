...
from src.modules.project.sections.tagging_config import (
    TAGGING_DEFAULTS,
    TAGGING_GUI_MODE_HEADING_MSGID,
    TAGGING_KEYS,
    TAGGING_LLM_MODES,
    TAGGING_MODES,
...
class TaggingSettingsTab(ttk.Frame):
    ...
    def __init__(self, parent: ttk.Frame, project_root: Path, **kwargs) -> None:
        ...
        self._postprocessing_frame: ttk.Frame | None = None
        self._postprocessing_row: int = 0
        self._hdr_postprocessing: ttk.Label | None = None
        self._intro_postprocessing: tk.Widget | None = None
        ...
        self._start_tags_warning_banner: tk.Frame | None = None
    ...
    def _build_ui(self) -> None:
        ...
        self._hdr_mode = gui_element_header_3(
            mode_row, locmsg(TAGGING_GUI_MODE_HEADING_MSGID), pack=False
        )
        self._hdr_mode.configure(wraplength=cfg["column_label_px"])
        block.finish_field_row(mode_row, self._hdr_mode, self._mode_combo)
        ...
        post_sub, self._postprocessing_frame, self._postprocessing_row = block.begin_sub_block()
        self._hdr_postprocessing = gui_element_header_3(
            self._postprocessing_frame,
            locmsg("project_tagging.section.postprocessing_algorithms"),
            pack=False,
        )
        post_sub.add_comment(self._hdr_postprocessing)
        self._intro_postprocessing = gui_element_input_description(
            self._postprocessing_frame,
            locmsg("project_tagging.postprocessing_intro"),
            wraplength=cfg["container_width_px"],
        )
        post_sub.add_comment(self._intro_postprocessing)

        row_ct = post_sub.add_field_row_frame()
        ...
        post_sub.finish_field_row(row_ct, self._lbl_create_tags, self._create_tags_combo)
        self._hint_create_tags = gui_element_input_description(
            self._postprocessing_frame,
            locmsg("project_tagging.field.create_tags_hint"),
            wraplength=cfg["container_width_px"],
        )
        post_sub.add_comment(self._hint_create_tags)

        self._tag_format_row_frame = post_sub.add_field_row_frame()
        ...
        post_sub.add_comment(self._tag_format_hint_widget)

        self._create_tags_field_var.trace_add(
            "write",
            lambda *_: (self._sync_field_bools_from_ui(), self._sync_create_tags_dependent_visibility()),
        )
        ...
        post_sub.add_label_row(self._lbl_start_tags)
        self._desc_start_tags = gui_element_input_description(
            self._postprocessing_frame,
            locmsg("project_tagging.hint.start_tags"),
            wraplength=cfg["container_width_px"],
        )
        post_sub.add_comment(self._desc_start_tags)
        self._start_tag_set_area = gui_element_input_text_area(
            self._postprocessing_frame, height_px=90
        )
        self._start_tag_set_area.set(TAGGING_DEFAULTS.start_tag_set or "")
        post_sub.add_full_width_row(self._start_tag_set_area)
        self._start_tags_warning_banner = gui_element_warning_banner(
            self._postprocessing_frame,
            locmsg("project_tagging.hint.start_tags_quote"),
            wraplength=cfg["container_width_px"],
        )
        post_sub.add_custom_row(
            self._start_tags_warning_banner,
            pady=UI_SETTINGS_BLOCK["warning_banner_row_pady"],
        )

        row_d = post_sub.add_field_row_frame()
        ...
        post_sub.finish_field_row(row_d, self._lbl_description, self._description_combo)
        self._hint_description = gui_element_input_description(
            self._postprocessing_frame,
            locmsg("project_tagging.field.description_hint"),
            wraplength=cfg["container_width_px"],
        )
        post_sub.add_comment(self._hint_description)

        row_dt = post_sub.add_field_row_frame()
        ...
        post_sub.finish_field_row(row_dt, self._lbl_date, self._date_combo)
        self._hint_date = gui_element_input_description(
            self._postprocessing_frame,
            locmsg("project_tagging.field.date_hint"),
            wraplength=cfg["container_width_px"],
        )
        post_sub.add_comment(self._hint_date)
        ...
        self._temperature_quote_banner = gui_element_warning_banner(
            self._model_settings_frame,
            locmsg("project_tagging.hint.temperature_quote"),
            wraplength=cfg["container_width_px"],
        )
    ...
    def _sync_create_tags_dependent_visibility(self) -> None:
        """Show tag format and start-tag fields only when «create tags» is enabled."""
        row = self._tag_format_row_frame
        hint = self._tag_format_hint_widget
        lbl_st = self._lbl_start_tags
        desc_st = self._desc_start_tags
        ban = self._start_tags_warning_banner
        area = self._start_tag_set_area
        ...
        if show:
            row.grid()
            hint.grid()
            if lbl_st is not None:
                lbl_st.grid()
            if desc_st is not None:
                desc_st.grid()
            if ban is not None:
                ban.grid()
            if area is not None:
    ...
    def _on_mode_change(self) -> None:
        ...
        show_post = code != TAGGING_MODES.skip
        if self._postprocessing_frame is not None:
            if show_post:
                grid_sub_block(self._postprocessing_frame, self._postprocessing_row)
            else:
                self._postprocessing_frame.grid_remove()
        ...
        try:
            ...
            if self._hdr_mode is not None:
                self._hdr_mode.configure(
                    text=locmsg(TAGGING_GUI_MODE_HEADING_MSGID),
                    wraplength=cfg["column_label_px"],
                )
            if self._hdr_postprocessing is not None:
                self._hdr_postprocessing.configure(
                    text=locmsg("project_tagging.section.postprocessing_algorithms"),
                    wraplength=cfg["container_width_px"],
                )
            if self._intro_postprocessing is not None:
                self._intro_postprocessing.configure(
                    text=locmsg("project_tagging.postprocessing_intro"),
                    wraplength=cfg["container_width_px"],
                )
            ...
            for banner, msgid in (
                (self._temperature_quote_banner, "project_tagging.hint.temperature_quote"),
                (self._start_tags_warning_banner, "project_tagging.hint.start_tags_quote"),
            ):
                if banner is not None and banner.winfo_exists():
                    ch = banner.winfo_children()
                    if len(ch) >= 2:
                        ch[1].configure(
                            text=locmsg(msgid),
                            wraplength=cfg["container_width_px"],
