"""Project list screen: open, create, and delete projects."""
...
class ProjectListScreen(BaseGUIScreen):
    """Project list: pick an existing project or create one."""

    SCREEN_CODE = "project_list"

    def get_screen_title(self) -> str:
        return locmsg("home.window_title")
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._topbar_btn_add: ttk.Button | None = None
        self._topbar_btn_settings: ttk.Button | None = None
        self._topbar_btn_models: ttk.Button | None = None
        self._build_ui()
    ...
    def _build_ui(self) -> None:
        ...
        self._page_title_label = gui_element_page_title(content_wrap, locmsg("home.title"))
    ...
    def refresh_locale(self) -> None:
        """Refresh top bar and page title after language change (rows via set_projects)."""
        try:
            if self._page_title_label is not None and self._page_title_label.winfo_exists():
                self._page_title_label.configure(text=locmsg("home.title"))
            for btn, key in (
                (self._topbar_btn_add, "gui.topbar.add_project"),
                (self._topbar_btn_settings, "gui.topbar.settings"),
                (self._topbar_btn_models, "gui.topbar.model_settings"),
            ):
                if btn is not None and btn.winfo_exists():
                    btn.configure(text=locmsg(key))
    ...
    def set_projects(self, projects: list[ProjectInfo]) -> None:
        """Replace the project list (called from the controller)."""
        ...
        if not self._projects:
            ttk.Label(self._projects_body, text=locmsg("home.empty_list"), style="Muted.TLabel").pack(
    ...
    def _top_panel(self) -> None:
        """Top bar: add project, settings, model settings."""
        ...
        self._topbar_btn_add = gui_element_button_primary(
            left_frame, locmsg("gui.topbar.add_project"), self._create_project
        )
        self._topbar_btn_add.pack(side=tk.LEFT, padx=(0, gh))
        right_frame = tk.Frame(top_bar, bg=bg)
        right_frame.pack(side=tk.RIGHT, padx=(0, ph), pady=pv)
        self._topbar_btn_settings = gui_element_button_secondary(
            right_frame, locmsg("gui.topbar.settings"), self._go_settings
        )
        self._topbar_btn_settings.pack(side=tk.LEFT, padx=(0, gh))
        self._topbar_btn_models = gui_element_button_secondary(
            right_frame, locmsg("gui.topbar.model_settings"), self._on_model_settings
        )
        self._topbar_btn_models.pack(side=tk.LEFT)
    ...
    def _build_project_row(self, info: ProjectInfo) -> None:
        ...
        files_lbl = tk.Label(
            left,
            text=self._files_label(info.docs_count),
            bg=p["bg_elevated"],
            fg=p["text_muted"],
        ...
        open_btn = gui_element_button_primary(
            row, locmsg("gui.project_row.select"), lambda p=info.path: self._open_project(p)
        )
        open_btn.grid(row=0, column=1, sticky=tk.E, padx=(0, 8), pady=6)

        open_folder_btn = gui_element_button_secondary(
            row, locmsg("gui.project_row.open_folder"), lambda p=info.path: open_folder(p)
        )
        open_folder_btn.grid(row=0, column=2, sticky=tk.E, padx=(0, 8), pady=6)

        delete_btn = gui_element_button_secondary(
            row, locmsg("gui.project_row.delete"), lambda i=info: self._ask_delete_project(i)
        )
    ...
    def _files_label(self, files_count: int) -> str:
        if files_count == 1:
            return locmsg("home.files.one")
        if files_count == 0:
            return locmsg("home.files.zero")
        return locmsg("home.files.many").format(count=files_count)
    ...
    def _ask_delete_project(self, info: ProjectInfo) -> None:
        ...
        self._app_layout.modals.show_confirm(
            title=locmsg("home.delete.title"),
            message=locmsg("home.delete.message").replace("{project_id}", str(info.id)),
            on_confirm=lambda: (self.on_delete_project(info) if self.on_delete_project else None),
            confirm_text=locmsg("gui.project_row.delete"),
            cancel_text=locmsg("gui.cancel"),
    ...
    def show_info(
        ...
        *,
        ...
        """Show an overlay info modal (e.g. from app after registry sync)."""
        if self._app_layout:
    ...
    def _create_project(self) -> None:
        ...
        self._app_layout.modals.show_input(
            title=locmsg("home.new_project.title"),
            prompt=locmsg("home.new_project.prompt"),
            on_submit=self._do_create_project,
            default="",
            submit_text=locmsg("gui.create"),
            cancel_text=locmsg("gui.cancel"),
        )
