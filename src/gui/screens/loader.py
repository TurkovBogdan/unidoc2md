"""Loading screen: first GUI view at startup."""
...
# Logo raster long side (Tk logical units; tk scaling applied in load_scaled_photoimage).
_LOADER_LOGO_MAX_SIDE_PT = 360


class LoadingScreen(BaseGUIScreen):
    """Shown while the app starts; optional first-run language pick."""
    ...
    def __init__(
        ...
        parent: ttk.Frame,
        *,
        on_language_ready: Callable[[], None] | None = None,
        **kwargs: object,
    ) -> None:
        self._on_language_ready = on_language_ready
    ...
    def _notify_locale_changed(self) -> None:
        """Like after settings save: refresh labels on screens already built."""
        try:
    ...
    def _build_ui(self) -> None:
        ...
        for code, label in AVAILABLE_LANGUAGES.items():
            ttk.Button(
                ...
                command=lambda c=code: self._on_language_chosen(c),
            ).pack(side=tk.LEFT, padx=(_gap, _gap))
