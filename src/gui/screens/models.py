"""Model settings screen: list view and navigation to per-model editor."""
...
from src.gui.template.elements.typography import gui_element_page_subtitle, gui_element_page_title
from src.gui.screens.base_screen import BaseGUIScreen
from src.gui.template.styles import GUI_CONTENT_WRAPPER, GUI_TOPBAR, UI_TYPOGRAPHY


class ModelSettingsScreen(BaseGUIScreen):
    """Model list with navigation to the detail editor screen."""

    SCREEN_CODE = "model_settings"

    def get_screen_title(self) -> str:
        return locmsg("models.window_title")

    _SUBTITLE_WRAP = 720
    _TITLE_TO_SUBTITLE_PAD = 0
    _SUBTITLE_PACK_PADY_TOP = 0
    ...
    def _build_ui(self) -> None:
        ...
        self._title_label = gui_element_page_title(content_wrap, locmsg("models.title"))
        self._title_label.pack_configure(pady=(0, self._TITLE_TO_SUBTITLE_PAD))
        self._subtitle_label = gui_element_page_subtitle(content_wrap, locmsg("models.intro"))
        self._subtitle_label.configure(wraplength=self._SUBTITLE_WRAP)
        _ps_after = UI_TYPOGRAPHY["page_subtitle"]["pady_after"]
        self._subtitle_label.pack_configure(pady=(self._SUBTITLE_PACK_PADY_TOP, _ps_after))
    ...
    def _top_panel(self) -> None:
        """Top bar: back, edit, sync registry."""
        ...
        self._back_btn.pack(side=tk.LEFT, padx=(0, gh))
        self._edit_btn = gui_element_button_primary(
            left_frame, locmsg("gui.edit"), self._open_selected_model
        )
        ...
        self._refresh_btn = gui_element_button_primary(
            right_frame, locmsg("gui.load_new_models"), self._on_refresh_registry
        )
        self._refresh_btn.pack(side=tk.LEFT)

    def refresh_locale(self, *, refresh_list_rows: bool = True) -> None:
        if self._back_btn is not None:
            self._back_btn.configure(text=locmsg("gui.back"))
        if self._edit_btn is not None:
            self._edit_btn.configure(text=locmsg("gui.edit"))
        if self._refresh_btn is not None:
            self._refresh_btn.configure(text=locmsg("gui.load_new_models"))
        if self._title_label is not None:
            self._title_label.configure(text=locmsg("models.title"))
        if self._subtitle_label is not None:
            self._subtitle_label.configure(text=locmsg("models.intro"))
        if self._model_list is not None:
            self._model_list.refresh_locale(refresh_rows=refresh_list_rows)

    def _get_manager(self):
        return get_llm_models_adapter(self.app_root)

    def _on_screen_show(self, event=None) -> None:
        """On show: refresh locale strings and reload the model list."""
        self.refresh_locale(refresh_list_rows=False)
    ...
    def _on_refresh_registry(self) -> None:
        ...
        def run() -> None:
            ...
            msg = locmsg("models.sync.message").format(
                providers_count=report.providers_count,
                models_new=report.models_new,
            )
    ...
    def _after_sync(self, message: str, errors: list[str] | None = None) -> None:
        ...
        self._reload_models(preferred_key=self._selected_model_key)
        self._show_info(locmsg("models.sync.modal_title"), message, errors=errors)
