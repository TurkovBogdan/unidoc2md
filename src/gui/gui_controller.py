"""Контроллер запуска GUI: экземплярный класс со статичным экземпляром, инкапсулирует инициализацию и mainloop."""
...
class GUIController:
    """Контроллер: запуск GUI, layout, экраны, привязка событий окна, mainloop. Один статичный экземпляр — через get()."""

    _instance: ClassVar[GUIController | None] = None

    @classmethod
    def init(cls, app_root: Path, root: tk.Tk) -> None:
        """Создаёт экземпляр, сохраняет в cls._instance, вызывает run()."""
    ...
    def get(cls) -> GUIController:
        """Возвращает текущий статичный экземпляр. Вызывать после init()."""
    ...
    def _on_close(self) -> None:
        """Обработчик закрытия окна: отвязка логгера, уничтожение root."""
    ...
    def _bind_window_events(self) -> None:
        """Добавляет обработчики событий окна (закрытие и т.п.)."""
        self._root.protocol("WM_DELETE_WINDOW", self._on_close)

    def _pump_ui(self) -> None:
        """Периодически выгружает очередь логов в консоль (вызывается по таймеру)."""
    ...
    def _show_screen(self, name: str) -> None:
        """Переключает отображаемый экран по имени и обновляет заголовок окна."""
        if self._router is not None:
            self._router.show_screen(name)

    def _show_project_list(self) -> None:
        """Показывает список проектов, при необходимости обновляет его."""
    ...
    def _open_model_settings_detail(self, model_key: str) -> None:
        """Открывает экран редактирования модели по ключу."""
    ...
    def _open_project(self, project_root: Path) -> None:
        """Открывает или создаёт экран конфигурации проекта и переключается на него."""
        ...
        if not self._registry.has(key):
            try:
                ...
                SYSTEM_LOGGER.exception("Ошибка открытия проекта %s: %s", project_root, exc)
                self._show_screen("project_list")
                project_list = self._registry.get("project_list")
                if project_list is not None and hasattr(project_list, "show_info"):
                    project_list.show_info("Ошибка открытия проекта", "", errors=[str(exc)])
                return
        else:
    ...
    def _show_project_pipeline(self, project_root: Path) -> None:
        """Показывает экран выполнения; запуск pipeline только по кнопке на этом экране."""
    ...
    def _setup_layout_and_console(self) -> None:
        """Создаёт layout, консоль логов, роутер, привязывает события и логгер."""
    ...
    def _build_screens(self) -> None:
        """Создаёт все экраны и регистрирует их в реестре."""
        from src.modules.project_pipeline import get_default_storage
        ...
        self._registry.register("loading", LoadingScreen(layout.loader_slot))
    ...
    def _start_ui(self) -> None:
        """Показывает загрузку, запускает насос логов и отложенный переход к списку проектов."""
        self._show_screen("loading")
        self._root.after(LOG_PUMP_INTERVAL_MS, self._pump_ui)
        self._root.after(1000, self._show_project_list)

    def run(self) -> None:
        """Layout, сборка экранов, привязка событий, mainloop. root уже создан и оформлен (GUIBootstrap.init)."""
        ...
        try:
            ...
            SYSTEM_LOGGER.exception("Ошибка в главном цикле GUI")
            raise
