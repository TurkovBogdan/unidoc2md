"""GUI-адаптеры для работы с реестром LLM-моделей."""
...
class LLMProviderModelRecord:
    """GUI-проекция записи реестра модели."""
...
class LLMModelsAdapter:
    """GUI-слой между экраном и доменным менеджером реестра моделей."""
    ...
    def list_models(self) -> list[LLMProviderModelRecord]:
        """Список моделей для GUI с фильтрацией по включённым провайдерам."""
    ...
    def get_model(self, model_key: str) -> LLMProviderModelRecord | None:
        """Одна запись модели из реестра по ключу provider@name."""
    ...
    def update_model(self, model_key: str, **fields: object) -> bool:
        """Обновляет модель как пользовательскую правку и блокирует bootstrap-merge."""
        return self._manager.update_user_model(model_key, **fields)

    def sync_available_models(self) -> SyncReport:
        """Синхронизация реестра с провайдерами для GUI."""
        try:
...
def get_llm_models_adapter(root: Path | None = None) -> LLMModelsAdapter:
    """Возвращает кешированный GUI-адаптер реестра моделей."""
