"""Модалка ввода строки: заголовок, подпись, поле ввода, «Отмена» и кастомная кнопка."""
...
class InputModal(OverlayModalBase):
    """Ввод одной строки: заголовок, подпись, поле ввода, «Отмена» и «Создать» (или кастомная кнопка)."""
    ...
    def show_input(
        ...
        *,
        ...
        if submit_text is not None:
            self._submit_btn.configure(text=submit_text)
