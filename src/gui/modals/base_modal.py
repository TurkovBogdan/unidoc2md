"""Базовый оверлей-модалка: затемнённый фон, карточка по центру."""
...
class OverlayModalBase(ttk.Frame):
    """Базовый оверлей: затемнённый фон, блокировка кликов, карточка по центру.

    Карточка без внутренних панелей, единый стиль по PALETTE.
    Подклассы переопределяют _build_content(card) для своего содержимого.
    """
    ...
    def _build_overlay(self) -> None:
        ...
        self._content.pack(
            ...
            padx=MODAL_CARD_PADX, pady=(MODAL_CARD_PADY_TOP, MODAL_CARD_PADY_BOTTOM),
        )
        ...
        self._card.place(
            ...
            width=MODAL_CARD_WIDTH, height=card_h,
        )
        self._card.bind("<Button-1>", lambda _e: "break")

    def _build_content(self, container: tk.Misc) -> None:
        """Переопределяется в подклассах: заголовок, текст, кнопки, поля ввода."""
        pass

    def _update_card_height(self) -> None:
        """Пересчитать высоту карточки по текущему контенту (нужно при динамическом контенте, например errors)."""
        self._content.update_idletasks()
        pady_total = MODAL_CARD_PADY_TOP + MODAL_CARD_PADY_BOTTOM
        req_h = self._content.winfo_reqheight()
        card_h = max(MODAL_CARD_MIN_HEIGHT, req_h + pady_total)
    ...
    def show(self) -> None:
        """Показывает модалку поверх родителя. Если родитель — слой модалок ( _is_app_modal_layer ), сначала показывается он."""
        parent = self.master
    ...
    def hide(self) -> None:
        """Скрывает модалку. Если родитель — слой модалок и других видимых модалок нет, скрывает слой."""
        toplevel = self.winfo_toplevel()
        try:
