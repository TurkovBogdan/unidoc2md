"""Поле выбора дробного числа с кнопками вверх/вниз в стиле системы."""
...
def _round_step(value: float, increment: float) -> float:
    """Округляет value до ближайшего кратного increment, чтобы избежать float-шума."""
    if increment <= 0:
        return value
    n = round(value / increment)
    return round(n * increment, 10)


class GuiInputSpinFloat(tk.Frame):
    """Поле выбора float с кнопками вверх/вниз (левая обводка, те же цвета)."""
    ...
    def _clamp_and_set(self) -> None:
        ...
        v = _round_step(v, self._increment)
        self._variable.set(self._format(v))
    ...
    def _step(self, direction: int) -> None:
        if self._state != "normal":
            return
        v = self._parse(self._variable.get() or "")
        v = v + direction * self._increment
        v = max(self._from, min(self._to, v))
        v = _round_step(v, self._increment)
        self._variable.set(self._format(v))
    ...
    def get_float(self) -> float:
        return self._parse(self._variable.get() or "")
