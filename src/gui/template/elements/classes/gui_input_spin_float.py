"""Float spinbox with up/down controls matching the theme."""
...
def _round_step(value: float, increment: float) -> float:
    """Round value to nearest multiple of increment to avoid float noise."""
    if increment <= 0:
        return value
...
class GuiInputSpinFloat(tk.Frame):
    """Float spin with up/down (themed left border and colors)."""
