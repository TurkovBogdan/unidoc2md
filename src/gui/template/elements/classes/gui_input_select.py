"""Кастомный селект без нативного Menu (без системных артефактов Windows)."""
...
class GuiInputSelect(tk.Frame):
    """Кастомный селект без нативного Menu, чтобы убрать системные артефакты Windows."""
    ...
    def _open_popup(self) -> None:
        ...
        # Отступ слева — префикс в тексте, чтобы подсветка выделения шла на всю строку
    ...
    def _select_from_popup(self, _event: tk.Event | None = None) -> str:
        ...
        # Короткий антидребезг: блокируем только «эхо»-клик после закрытия попапа.
    ...
    def _on_wheel_select(self, event: tk.Event) -> str:
        """Переключение выбранного элемента колёсиком только при открытом попапе. Иначе не перехватывать — скролл страницы."""
        if self._popup is None or not self._popup.winfo_exists():
            return
    ...
    def _on_wheel_select_linux(self, event: tk.Event) -> str:
        """Переключение выбранного элемента колёсиком только при открытом попапе (Linux: Button-4/5)."""
        if self._popup is None or not self._popup.winfo_exists():
            return
        if event.num == 4:
            self._wheel_step(-1)
    ...
    def _wheel_step(self, step: int) -> None:
        """Сменить выбранное значение на step: -1 = предыдущий, +1 = следующий."""
