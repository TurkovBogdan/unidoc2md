"""Инициализация стилей кнопок: TButton, Primary.TButton, Secondary.TButton, курсор при наведении."""
...
def gui_setup_button_styles(
    ...
    """
    Стили кнопок: TButton — база (общие параметры + primary-цвета);
    Primary.TButton наследует TButton; Secondary.TButton — только свои цвета.
    Состояние disabled: UI_COLOR_DISABLED / UI_COLOR_DISABLED_TEXT из theme.py.
    Отступы и рамка из UI_BUTTONS, шрифт передаётся из темы.
    """
    p = palette
    b = UI_BUTTONS
    _btn_base = {
    ...
    # Primary — как в элементах; без parent не подхватывает map TButton (clam по умолчанию).
    try:
