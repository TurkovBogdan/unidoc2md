"""
Применение темы к корневому окну: настройка ttk-стилей, шрифтов, цветов.
Класс GUITemplate: создаётся экземпляр, apply_theme(root) применяет тему.
"""
...
class GUITemplate:
    """Применение единой темы к корневому окну Tk (ttk-стили, шрифты, цвета)."""

    def apply_theme(self, root: tk.Tk) -> None:
        """
        Применяет единую чёрно-тёмно-зелёную тему к ttk-виджетам.
        Шрифты берутся из GUIConfigStore (заполняется в GUIBootstrap._prepare).
        """
        ...
        # Название поля + комментарий (UI_INPUT_LABEL_DESCRIPTION)
        ...
        # Заголовок правой панели (Пояснения / Полный промпт): без горизонтального отступа, в одну линию с текстом
        style.configure(
        ...
        # Элементы страницы: заголовок, подзаголовок, заголовки 2/3 уровня (из UI_TYPOGRAPHY)
        _style_header = (
        ...
        # Заголовок модалки: как header_2, но фон под карточку (bg_elevated)
