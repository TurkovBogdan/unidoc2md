"""Tab switcher styled to match the design system (PALETTE)."""
...
import src.gui.template.styles.theme as _theme  # font resolved at runtime (after gui_setup_theme)


class StyledTabView(ttk.Frame):
    """
    Tab bar plus content area.
    Colors and padding from UI_TABS (template/styles). 1px line under tabs uses active tab color.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        Args:
            ...
            tabs: List of (tab_id, label), e.g. [("general", "General"), ("extract", "Extract")].
            initial: Default tab id (if None, first tab is used).
            on_tab_change: Optional callback on tab change: on_tab_change(tab_id).
        """
        super().__init__(parent, **kwargs)
        self._tabs = list(tabs)
        self._on_tab_change = on_tab_change
        self._current: str | None = None
        self._pressed_tab_id: str | None = None  # tab where mouse button is held
    ...
    def _build_tab_bar(self) -> None:
        ...
        # Line under tab bar (active tab color)
    ...
    def add_tab_content(self, tab_id: str, frame: tk.Misc) -> None:
        """
        Attach content frame to a tab.
        Frame must use parent=view.content_holder.
        Call once per tab after StyledTabView is created.
        """
    ...
    def update_tab_labels(self, label_by_id: dict[str, str]) -> None:
        """Update tab labels by id (e.g. after language change)."""
        for i, (tab_id, _old) in enumerate(self._tabs):
            if tab_id in label_by_id:
                self._tabs[i] = (tab_id, label_by_id[tab_id])
        for tab_id, _tab_frame, lbl in self._tab_widgets:
            new_text = label_by_id.get(tab_id)
