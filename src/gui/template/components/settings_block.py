"""
Блок настроек: единое место для разметки и отступов.
Все grid(..., pady=..., padx=...) задаются только здесь, значения — из UI_SETTINGS_BLOCK.
Вызывающий код только создаёт виджеты и вызывает методы блока; отступы всегда одинаковые.
"""
...
class _RowBuilder:
    """Внутренний построитель строк: общая логика отступов для form и для вложенного frame."""
    ...
    def add_field_row_frame(self) -> ttk.Frame:
        """Создаёт строку (лейбл|поле) и возвращает фрейм. Вызвать finish_field_row(frame, label, field) после создания виджетов с parent=frame."""
        c = self._cfg
    ...
    def finish_field_row(
        ...
        """Размещает лейбл и поле во фрейме строки. Отступы только из cfg."""
        c = self._cfg
    ...
    def add_comment(self, widget: tk.Widget | None) -> None:
        """Строка-описание на всю ширину. Отступы только из cfg."""
        if widget is None:
            return
        widget.grid(
    ...
    def add_label_row(self, label_widget: tk.Widget) -> None:
        """Строка только с лейблом (без двух колонок). Для размещения над full-width виджетом."""
        label_widget.grid(
    ...
    def add_full_width_row(self, widget: tk.Widget) -> None:
        """Строка с виджетом на всю ширину (например, текстовое поле). Отступы как у полей."""
        widget.grid(
            row=self._row, column=0, sticky=tk.EW,
            pady=self._field_row_pady(),
        )
...
class SettingsBlock(_RowBuilder):
    """
    Блок настроек. Весь grid и все pady/padx задаются здесь из UI_SETTINGS_BLOCK.
    Вызывающий код не вызывает .grid() с отступами.
    """
    ...
    def add_intro_banner(self, widget: tk.Widget) -> int:
        """Первый баннер. Возвращает номер строки (для последующего show/hide)."""
    ...
    def add_section_banner(self, widget: tk.Widget) -> int:
        """Баннер начала секции (block_sep). Возвращает номер строки для show/hide."""
    ...
    def begin_sub_block(self) -> tuple[_RowBuilder, ttk.Frame, int]:
        """Вложенный блок (OCR/Vision). Возвращает (построитель, фрейм, номер строки для show/hide)."""
        c = self._cfg
...
def grid_section_banner(widget: tk.Widget, row: int) -> None:
    """Показать баннер секции снова (после grid_remove). Отступы из UI_SETTINGS_BLOCK."""
    c = UI_SETTINGS_BLOCK
...
def grid_sub_block(frame: tk.Widget, row: int) -> None:
    """Показать подблок (OCR/Vision) снова (после grid_remove). Отступы из UI_SETTINGS_BLOCK."""
    c = UI_SETTINGS_BLOCK
