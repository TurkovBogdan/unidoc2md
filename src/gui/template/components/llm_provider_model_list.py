"""LLM provider model list with filters (read-only)."""
...
class LLMProviderModelList(ttk.Frame):
    """Left pane: filters and model list (no inline editing)."""
    ...
    def __init__(self, parent: tk.Misc, *, on_select=None, on_activate=None, **kwargs) -> None:
        ...
        self._all_label = locmsg("models.provider_all")
        self._provider_values: tuple[str, ...] = (self._all_label,)
        self._provider_var = tk.StringVar(value=self._all_label)
        self._search_var = tk.StringVar()
        self._provider_label: ttk.Label | None = None
        self._search_label: ttk.Label | None = None
    ...
    def _build_ui(self) -> None:
        ...
        self._provider_label = ttk.Label(
            filters, text=locmsg("models.filter.provider"), anchor=tk.W
        )
        self._provider_label.grid(
            row=0,
            column=0,
            sticky=tk.W,
            padx=(0, SPACING["sm"]),
        ...
        self._search_label = ttk.Label(filters, text=locmsg("models.filter.search"), anchor=tk.W)
    ...
    def _apply_tree_headings(self) -> None:
        if not self._tree:
            return
        self._tree.heading("provider", text=locmsg("models.col.provider"))
        self._tree.heading("model", text=locmsg("models.col.model"))
        self._tree.heading("enabled", text=locmsg("models.col.enabled"))
        self._tree.heading("text", text=locmsg("models.col.text"))
        self._tree.heading("image", text=locmsg("models.col.image"))
        self._tree.heading("reasoning", text=locmsg("models.col.reasoning"))
        self._tree.heading("price", text=locmsg("models.col.price"))
        self._tree.heading("created", text=locmsg("models.col.created"))

    def refresh_locale(self, *, refresh_rows: bool = True) -> None:
        old_all = self._all_label
        self._all_label = locmsg("models.provider_all")
        if self._provider_label is not None:
            self._provider_label.configure(text=locmsg("models.filter.provider"))
        if self._search_label is not None:
            self._search_label.configure(text=locmsg("models.filter.search"))
        self._apply_tree_headings()
        cur = self._provider_var.get()
        if cur == old_all:
            self._provider_var.set(self._all_label)
        rest = self._provider_values[1:] if len(self._provider_values) > 1 else ()
        self._provider_values = (self._all_label,) + rest
        if self._provider_select is not None:
            self._provider_select.set_values(self._provider_values)
        if self._provider_var.get() not in self._provider_values:
            self._provider_var.set(self._all_label)
        if refresh_rows:
            self._apply_filters(preferred_key=self.get_selected_key())

    def _bool_label(self, value: bool) -> str:
        return locmsg("models.bool.yes") if value else locmsg("models.bool.no")
    ...
    def set_models(
        ...
        *,
        ...
        provider_values = tuple([self._all_label] + sorted({m.provider_code for m in self._models}))
    ...
    def _matches_filters(self, model: LLMProviderModelRecord) -> bool:
        ...
        if provider and provider != self._all_label and model.provider_code != provider:
            return False
