...
MARKDOWN_LOGIC_REBUILD_TAGS = "rebuild_tags"
MARKDOWN_LOGIC_REBUILD_DOCUMENT = "rebuild_document"


class MarkdownExtractProvider(FileExtractProvider):
    """Extract .md as one content block (markdown or text) per selected algorithm."""

    # Заголовок блока на вкладке «Извлечение текста» (не этап пайплайна): project_extract.provider.<code>.title
    ...
    def project_settings_schema(cls) -> tuple[SettingFieldSchema, ...]:
        return (
            SettingFieldSchema(
                key="algorithm",
                type="select",
                default=MARKDOWN_LOGIC_REBUILD_TAGS,
                label="algorithm",
                description="",
                options=(
                    (MARKDOWN_ALGORITHM_SKIP, MARKDOWN_ALGORITHM_SKIP),
                    (MARKDOWN_LOGIC_SAVE_AS_IS, MARKDOWN_LOGIC_SAVE_AS_IS),
                    (MARKDOWN_LOGIC_REBUILD_TAGS, MARKDOWN_LOGIC_REBUILD_TAGS),
                    (MARKDOWN_LOGIC_REBUILD_DOCUMENT, MARKDOWN_LOGIC_REBUILD_DOCUMENT),
                ),
            ),
        )
    ...
    def extract(
        ...
        logic = (self.get_setting(config, "algorithm") or MARKDOWN_LOGIC_REBUILD_TAGS).strip().lower()
        ...
        if logic == MARKDOWN_LOGIC_SAVE_AS_IS:
            pass
        elif logic == MARKDOWN_LOGIC_REBUILD_TAGS:
            text = clear_markdown_yaml(text)
        elif logic == MARKDOWN_LOGIC_REBUILD_DOCUMENT:
            text = clear_markdown_yaml(text)

        content: list[ExtractedDocumentContent] = []
        if logic == MARKDOWN_LOGIC_SAVE_AS_IS or logic == MARKDOWN_LOGIC_REBUILD_TAGS:
            content_type = CONTENT_TYPE_MARKDOWN
            semantic_type = SEMANTIC_TYPE_MARKDOWN
            ext = ".md"
        else:
            assert logic == MARKDOWN_LOGIC_REBUILD_DOCUMENT
